...
def calculate_square_sum(x: np.array):
    return sum([i * i for i in x])


def compare_modes(empirical_modes: pd.Series):
    etalon = calculate_square_sum(etalon_data.get_etalon_emd())
    current = calculate_square_sum(empirical_modes)
